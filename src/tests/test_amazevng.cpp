...
int main() {
    ...
    // 5) Demosaic (AMaZE-VNG 混合算法, BGGR -> BGR, 保持 16-bit)
    cv::Mat color16;
    demosiacAMaZEVNG(raw, color16, BGGR);
    std::cout << "Demosaic done (AMaZE-VNG Hybrid)." << std::endl;
    ...
    // 10) 输出
    std::string outFile = "data/output/raw4_pipeline_amazevng_gamma.png";

    if (cv::imwrite(outFile, color8_gamma)) {
        std::cout << "Saved (AMaZE-VNG Hybrid + gamma, 8-bit): " << outFile << std::endl;
    } else {

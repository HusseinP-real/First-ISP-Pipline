...
int main() {
    ...
    // 5) Demosaic (LMMSE, BGGR -> BGR, 保持 16-bit)
    cv::Mat color16;
    const float sigma_noise = 100.0f;  // 噪声标准差估计值
    demosiacLMMSE(raw, color16, BGGR, sigma_noise);
    std::cout << "Demosaic done (LMMSE, sigma_noise=" << sigma_noise << ")." << std::endl;
    ...
    // 10) 输出
    std::string outFile = "data/output/raw5_pipeline_lmmse_gamma.png";

    if (cv::imwrite(outFile, color8_gamma)) {
        std::cout << "Saved (LMMSE + gamma, 8-bit): " << outFile << std::endl;
    } else {

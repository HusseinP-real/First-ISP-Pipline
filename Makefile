...
# 源文件
CORE_SOURCES = $(CORE_DIR)/raw_reader.cpp $(CORE_DIR)/blc.cpp
TEST_SOURCES = $(TESTS_DIR)/test_raw_reader.cpp $(TESTS_DIR)/test_blc.cpp

# 目标文件
CORE_OBJECTS = $(BUILD_DIR)/raw_reader.o $(BUILD_DIR)/blc.o
TEST_TARGET = $(BUILD_DIR)/test_raw_reader
TEST_BLC_TARGET = $(BUILD_DIR)/test_blc

# 默认目标
all: $(TEST_TARGET) $(TEST_BLC_TARGET)
...
.PHONY: all run run-blc clean help
